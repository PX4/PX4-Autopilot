...
VIDEO_FILE = '../RRVflights/2019_08_10/flight1.MTS';
ESC_FILE = '../RRVflights/2019_08_10/flight1_ESC.mat';
LOG_FILE = '../RRVflights/2019_08_10/flight1.mat';
OUTPUT_VIDEO_FILE = '../RRVflights/2019_08_11/flight0.mp4';
...
roll = data.ATT_Roll;
pitch = data.ATT_Pitch;
yawrate = data.ATT_YawRate;
rc_yaw = data.RC_C2;
rc_pitch = data.RC_C1;
rc_roll = data.RC_C0;
thr = data.RC_C3;
airspeed = data.AIRS_TrueSpeed;
...
if ( ~exist('throttle_pix_index','var') || throttle_pix_index < 0 )
    ...
    subplot(2,1,2);
    plot(1:length(ESC_throttle),ESC_throttle);
    ylabel('Throttle');
    title('ESC');
...
% init time for ESC
t_esc = zeros(length(ESC_throttle),1);
...
ESC_throttle(isnan(ESC_throttle)) = 0;
ESC_RPM(isnan(ESC_RPM)) = 0;
...
ESC_throttlef = interp1(t_esc,ESC_throttle,t);
ESC_RPMf = interp1(t_esc,ESC_RPM,t);
