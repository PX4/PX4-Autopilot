...
requirement_defns['mode_req_global_position_relaxed'] = {'text': "Position measurement updates in a global coordinate frame but accepts poor accuracy", 'sensor': "", "detail": "Local position and global reference to EKF 0" } 
requirement_defns['mode_req_local_alt'] = {'text': "Local altitude relative to EKF 0 position", 'sensor': "Barometer corrected by GNSS altitude over time, distance sensor", "detail": "Usually not the distance sensor because if the ground shifts then the coordinate frame would shift with it (range aid problem)."} 
...
requirement_defns['mode_req_prevent_arming'] = {'text': "Mode must be armable", 'sensor': "", "detail": "Set for modes like Land, Orbit, Return that you can't take off in" } 
...
output_path = repo_root / "docs/en/flight_modes"
...
{requirement_headings}
    ...
    for vehicle_type in vehicle_modes_sorted:
        for flight_mode in vehicle_modes_sorted[vehicle_type]:
            ...
            vehicle_mode_filename = output_path / vehicle_mode_filename
            print(vehicle_mode_filename)
...
"""
            ...
            for requirement in vehicle_modes_sorted[vehicle_type][flight_mode]:
                ...
                #vehicle_mode_markdown += f"- {text} ([`{requirement}`](xxx)):\n  - {detail}\n"

            try:
                with open(vehicle_mode_filename, 'w', encoding='utf-8') as f:
                    # 'w' stands for 'write' mode, which creates the file or overwrites an existing one.
                    f.write(vehicle_mode_markdown)
                
                print(f"✅ Successfully wrote content to {vehicle_mode_filename}")
