...
- Always do your first bench tests with the propeller removed.
...
- **Electric starter** on an output, typically through a relay or an ESC driving the starter motor.
  Optionally, the engine can also be hand-started.
- **RPM sensor** feeding the `RPM Input` function on a timer-capture-capable PWM pin, for example a Hall-effect or optical sensor on the crankshaft, or an ignition-pulse pickup.
  ...
  Many engines have a built-in spark-timing sensor that can be used for this.
  If yours does not, see [Tachometers](../sensor/tachometers.md) for other options.
...
![Actuator setup for ICE, with the engine output functions and the RPM input highlighted](../../assets/hardware/ice/ice_actuator_setup.png)
...
The throttle output also goes to _Disarmed_ while the engine is running if the [idle governor](#idle-rpm-governor) is disabled and the throttle demand drops to near zero (the fixed-wing controller stops the motor below 2% thrust).
:::

Points to note when setting the values:

- `IC Engine Throttle` uses **Minimum** for closed throttle and **Maximum** for full throttle.
  Setting **Disarmed** to a value other than **Minimum** parks the servo at an idle stop when the engine is off, rather than at closed throttle.
- `IC Engine Choke` uses **Minimum** for choke open and **Maximum** for choke closed; tick **Rev Range** if the servo linkage is inverted.
...
::: tip
In the example above the **Actuators** tab stays red in the sidebar, and the page shows "One or more actuator still needs to be assigned to an output".
This is expected for an engine setup and does not stop the engine from running: the airframe's `Motor 1` slot is deliberately left unassigned, because the throttle reaches the servo through `IC Engine Throttle` instead — see [How It Works](#how-it-works).
:::
...
The engine can only be commanded when [armed or prearmed](../advanced_config/prearm_arm_disarm.md).
:::
...
This dead zone means a three-position switch has a middle position that does not change the engine state, and it prevents a channel that is momentarily centred (or lost and defaulted to zero) from cutting the engine.
:::
...
- [ICE_IDLE_RPM_P](../advanced_config/parameter_reference.md#ICE_IDLE_RPM_P) is the normalised throttle added per **1000 RPM** of error.
  `ICE_IDLE_RPM_P = 1` means an engine running 500 RPM slow gets `+0.5` throttle.
- [ICE_IDLE_RPM_I](../advanced_config/parameter_reference.md#ICE_IDLE_RPM_I) is the normalised throttle added per **1000 RPM·s** of accumulated error.
...
<img src="../../assets/hardware/ice/ice_control_diagram.svg" alt="ICE control architecture" class="diagram-invert">

The module sits between the [control allocator](../config/actuators.md) and the outputs:

- It takes the **throttle demand** from `actuator_motors.control[0]`, i.e. the thrust that allocation computed for `Motor 1`.
  ::: info
  The engine takes its throttle from whichever rotor the airframe geometry defines as `Motor 1` (rotor 0, `CA_ROTOR0_*`), so you must set up the geometry with the engine as rotor 0.
  On a standard VTOL, this means making the pusher rotor 0 rather than the last rotor.
  You do not need to (and should not) assign an output for the `Motor 1` function.
  :::
- It takes the **on/off request** from [VehicleStatus](../msg_docs/VehicleStatus.md) (arming state, or the VTOL transition) or from [ManualControlSetpoint](../msg_docs/ManualControlSetpoint.md) (the AUX channels), depending on [ICE_ON_SOURCE](#start-stop-command-source).
...
<img src="../../assets/hardware/ice/ice_control_state_machine.svg" alt="ICE control state machine" class="diagram-invert">
...
Both `Starting` and `Running` have sub-states, reported in `InternalCombustionEngineStatus.substate`.
`Starting` alternates between cranking and a one-second rest between attempts (see [Start Sequence](#start-sequence)), and `Running` switches between passing the throttle demand straight through and running the [idle governor](#idle-rpm-governor).
