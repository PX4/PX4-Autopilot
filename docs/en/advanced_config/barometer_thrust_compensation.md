...
```
corrected_baro_alt = raw_baro_alt + SENS_BARO_K_T * |thrust_z|
```

where `thrust_z` is the Z body-axis component of `vehicle_thrust_setpoint` (negative for upward thrust in FRD frame).

The compensation parameter [SENS_BARO_K_T](../advanced_config/parameter_reference.md#SENS_BARO_K_T) can be identified automatically during flight or manually from a flight log.
...
3. Run the analysis script on the log:
   ...
   ```
4. Apply the recommended `SENS_BARO_K_T` value.
