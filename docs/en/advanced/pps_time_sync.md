...
PPS time synchronization is supported on flight controllers that have:
- A hardware timer input pin that can be configured for PPS capture
- The PPS capture driver enabled in the board configuration
...
### 1. Enable PPS Driver in Board Configuration

The PPS capture driver must be enabled in the board configuration.
...
1. Connect to the PX4 console (via MAVLink shell or serial console)
2. Wait for GPS fix
3. Check the PPS capture status:
...
You can also check the [PpsCapture](../msg_docs/PpsCapture.md) uORB topic
...
- Integration with the GPS driver for refined time stamping
...
5. This offset is used to correct GPS timestamps and improve sensor fusion accuracy
...
**Note**: If the PPS driver is not sending any data for 5 seconds despite having `PPS_CAP_ENABLE` set to 1, the `EKF2_GPS_DELAY` is used again
for estimating the latency.
