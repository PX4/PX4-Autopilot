...
PX4 provides several approaches for finding a path to a safe destination and/or landing, selected using the [RTL_TYPE](#RTL_TYPE) parameter.
...
- [Route safe point return](#rtl_type_6) (`RTL_TYPE=6`): Join the uploaded mission route, follow it towards the lowest-cost rally point, branch off close to that point, and land.
...
::: warning
This return type does not have [geofence awareness](#geofence_awareness) (at any stage).
:::
...
### Route Safe Point Return Type (RTL_TYPE=6) {#rtl_type_6}

This return type uses an uploaded mission as the route towards a rally (safe) point instead of flying directly to the destination.

PX4 plans and executes the return as follows:

1. Projects the vehicle and each usable rally point onto the mission route.
2. Selects the lowest-cost destination using the distance along the route, the direct branch-off leg to the rally point, and a configurable fixed-wing U-turn penalty.
3. Joins the route at the vehicle projection and follows it in the selected direction.
4. Leaves the route at the selected branch-off point and lands at the rally point.

If no rally point is usable, PX4 instead follows the route towards the closer available mission takeoff or landing endpoint and lands there.
While following the route, PX4 treats the mission as geometry: `DO_JUMP` control flow, payload commands, and other non-position items are skipped, while loiter and hold items are converted to continuous waypoints.
Takeoff and landing endpoints retain their dedicated behavior.

::: info
This behavior is specific to Route Safe Point Return.
There is no separate setting that enables smart route joining during normal Mission mode.
:::

::: warning
This mode requires a complete mission from the [mission route cache](../advanced/mission_route_cache.md).
If the cache is disabled, still loading, too small for the mission, or planning otherwise fails, PX4 uses the direct closest-safe-destination behavior of [`RTL_TYPE=3`](#rtl_type_3).
:::

::: warning
VTOL vehicles currently use the same direct fallback because front transitions are not yet enabled in `AUTO_RTL`.
Route-following VTOL support will be added separately.
:::

Once the selected approach or landing has started, PX4 completes that landing instead of switching back to a newly available route plan.

::: warning
The planned route, join leg, and branch-off leg are not checked against geofences.
The direct `RTL_TYPE=3` fallback retains its normal [geofence-aware behavior](#geofence_awareness).
:::

The route search is configured with:

- [MIS_MC_SEG_DIST](../advanced_config/parameter_reference.md#MIS_MC_SEG_DIST): Vehicle projection search margin for multicopters.
- [MIS_FW_SEG_DIST](../advanced_config/parameter_reference.md#MIS_FW_SEG_DIST): Vehicle projection search margin for fixed-wing aircraft.
- [RTL_RP_SEG_DIST](../advanced_config/parameter_reference.md#RTL_RP_SEG_DIST): Rally-point projection search margin.
- [RTL_FW_UTURN_PEN](../advanced_config/parameter_reference.md#RTL_FW_UTURN_PEN): Extra cost applied when a fixed-wing route starts with a U-turn.

For planner and cache configuration details, see [Mission Route Planning](../concept/mission_route_planning.md) and [Mission Route Cache](../advanced/mission_route_cache.md).
