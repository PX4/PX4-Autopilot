...
Offboad mode through ROS 2 is intrinsically unsafe - please read carefully this page to fully understand the risks involved when using it.
A good understanding of [PX4 controller diagrams](../flight_stack/controller_diagrams.md) is advised.

Check-out the [PX4 ROS 2 Interface Library](../ros2/px4_ros2_interface_lib.md) for a safer alternative.
...
If the rate falls below 2Hz while under external control PX4 will switch out of offboard mode after a timeout ([COM_OF_LOSS_T](#COM_OF_LOSS_T)), and attempt to land or perform some other failsafe actions.
...
This section describes how to perform offboard control through one of the direct ROS 2 interface: UXRCE-DDS or Zenoh.

When using direct ROS 2 offboard control PX4 setpoint messages generated by external controllers are forcefully injected inside the [PX4 control pipeline](../flight_stack/controller_diagrams.md).
At the time of writing PX4 has no means of filtering and distinguishing them from the internal setpoing messages.
This requires **precise synchronization** between the external controllers and the internal ones.

::: warning

- The external controller shall publish PX4 setpoint messages **IF AND ONLY IF** PX4 mode is Offboard.
- If external setpoints are sent in any other flight mode they will overlap with the internal ones causing unexpected behaviors.
- If external setpoints are NOT sent when PX4 mode is Offboard, PX4 won't trigger any failsafe actions causing unexpected behaviors.
...
- [px4_msgs::msg::TrajectorySetpoint](https://github.com/PX4/PX4-Autopilot/blob/main/msg/versioned/TrajectorySetpoint.msg)
  ...
  Position, velocity and acceleration control for multicopter all all handled by the `mc_pos_control` module.
...
For rover-specific control setpoints and better behavior we recommend using the [Rover Setpoints](#rover-setpoints) via ROS 2.
