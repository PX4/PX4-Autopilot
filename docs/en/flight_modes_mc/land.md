...
| <a id="MPC_AUTO_RC_YAW"></a>[MPC_AUTO_RC_YAW](../advanced_config/parameter_reference.md#MPC_AUTO_RC_YAW) | Enable yaw stick nudging of vehicle heading in Auto modes (multicopter), held when the stick is released. During landing this applies only when [MPC_LAND_RC_HELP](#MPC_LAND_RC_HELP) is disabled. Requires stick override to be disabled ([MAN_OVERRIDE_SPD](#MAN_OVERRIDE_SPD) = -1). |
| <a id="MPC_LAND_RC_HELP"></a>[MPC_LAND_RC_HELP](../advanced_config/parameter_reference.md#MPC_LAND_RC_HELP) | Enable stick nudging during the autonomous land routine (multicopter): the vehicle can be moved horizontally, yawed, and have its descent speed adjusted with the sticks. When enabled it takes precedence over [MPC_AUTO_RC_YAW](#MPC_AUTO_RC_YAW) for yaw. Requires stick override to be disabled ([MAN_OVERRIDE_SPD](#MAN_OVERRIDE_SPD) = -1). |
