...
[SYS_HAS_NUM_GNSS](#SYS_HAS_NUM_GNSS) also controls arming behaviour when the EKF2 GNSS quality checks (configured via [EKF2_GPS_CHECK](../advanced_config/parameter_reference.md#EKF2_GPS_CHECK)) are failing: when set to require one or more receivers, a failing check blocks arming.

| Parameter                                                                                                   | Description                                                                                                                                       |
| ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| <a id="SYS_HAS_NUM_GNSS"></a>[SYS_HAS_NUM_GNSS](../advanced_config/parameter_reference.md#SYS_HAS_NUM_GNSS) | Number of usable GNSS receivers required for arming and flight. If two are required then they also need to be consistent. When `> 0`, also blocks arming on EKF2 GNSS quality check failures. |
| <a id="COM_GNSSLOSS_ACT"></a>[COM_GNSSLOSS_ACT](../advanced_config/parameter_reference.md#COM_GNSSLOSS_ACT) | Failsafe action when a GNSS failure is detected.                                                                                                                                              |
...
| <a id="COM_ARM_WO_GPOS"></a>[COM_ARM_WO_GPOS](../advanced_config/parameter_reference.md#COM_ARM_WO_GPOS)    | Allow arming without a global position estimate. `0`: Deny arming, `1`: Allow with warning (default), `2`: Allow without warning.                                                                          |
