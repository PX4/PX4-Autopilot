...
- **Secure the link below MAVLink**, using a radio configured with link-layer security, a VPN, etc.
  Configured with authenticated encryption, this uses standard, reviewed cryptography and gives confidentiality and authentication for all traffic on the link, not only MAVLink.
  It is outside PX4's scope and depends on your radio and network stack.
