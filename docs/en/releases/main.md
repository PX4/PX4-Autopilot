...
- ADS-B conflict detection: removed the `navigator fake_traffic` CLI command and its underlying `AdsbConflict::fake_traffic()`/`run_fake_traffic()` methods. The buffer lifecycle behaviour is now covered by an automated unit test. (PX4-Autopilot#<!-- PR number -->)
