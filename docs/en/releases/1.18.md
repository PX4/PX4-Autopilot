...
12. **Update AFBR-S50 rangefinder parameters.**
    `SENS_AFBR_S_RATE`, `SENS_AFBR_L_RATE`, `SENS_AFBR_THRESH` and `SENS_AFBR_HYSTER` are removed together with the distance-based range switching. Rate and dual-frequency mode are set via [SENS_AFBR_RATE](../advanced_config/parameter_reference.md#SENS_AFBR_RATE), [SENS_AFBR_DFM](../advanced_config/parameter_reference.md#SENS_AFBR_DFM) and [SENS_AFBR_PROF](../advanced_config/parameter_reference.md#SENS_AFBR_PROF); the defaults select per-module values. [SENS_AFBR_MODE](../advanced_config/parameter_reference.md#SENS_AFBR_MODE) now defaults to `-1` (module default mode).
...
- AFBR-S50 rangefinder: static per-module rate/DFM profiles ([SENS_AFBR_PROF](../advanced_config/parameter_reference.md#SENS_AFBR_PROF)), signal quality gate ([SENS_AFBR_QMIN](../advanced_config/parameter_reference.md#SENS_AFBR_QMIN)), invalid frames reported as out of range, and on-demand range offset calibration (`afbrs50 cal`). ([PX4-Autopilot#28454](https://github.com/PX4/PX4-Autopilot/pull/28454))
