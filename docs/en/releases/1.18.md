...
- [Serial Passthrough (MAVLink SERIAL_CONTROL)](../uart/serial_passthrough.md): new `serialpassthrough` driver enables MAVLink clients to read from and write to FC serial ports (`TELEM1`/`TELEM2`, `GPS1`/`GPS2`, `TELEM3`/`TELEM4`).
  On STM32F7/H7 boards, ESC signal pins can also be written via a software bit-bang UART (device IDs 20–27).
  Host tools connect through a bridge application, such as [`Tools/mavlink_serial_bridge.py`](https://github.com/PX4/PX4-Autopilot/blob/main/Tools/mavlink_serial_bridge.py) (it isn't part of the v1.18 release, but the version on `main` works with v1.18 firmware).
