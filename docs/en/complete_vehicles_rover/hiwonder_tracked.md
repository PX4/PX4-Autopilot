...
3. Choose **Hiwonder Tracked** under the **Rover** category.
...
1. Select the Hiwonder EMM driver from the list of _Actuator Outputs_.
   ...
   Assign the one of the populated channels of the motor controller board to the `Left Motor` and one to the `Right Motor`. The channels are noted on the motor controller board (alternatively randomly assign the channels and use the actuator testing tab to find the correct assignments).
