...
`VTX_DEVICE` selects the specific VTX model and only enables device-specific workarounds.
It is not a hardware compatibility list: a VTX that is not listed works fine with `VTX_DEVICE` = `0`.
