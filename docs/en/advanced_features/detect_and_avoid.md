...
The detailed equations are in [F3442 Volume Appendix](#f3442-volume-appendix).
...
`NAV_TRAFF_AVOID` and `DAA_LVL_*_ACT` share the same mapping for `Disabled`, `Warn only`, `Return`, `Land`, `Hold`, and `Terminate`.
...
Navigator commands for actionable levels are:

- `Hold`: switch to Auto Loiter.
- `Return`: trigger RTL and enforce the minimum return altitude.
- `Land`: trigger Auto Land.
- `Terminate`: trigger flight termination.

#### Conflict-to-Action Mapping

- `Crosstrack` only publishes `HIGH`, so its requested action comes directly from [NAV_TRAFF_AVOID](../advanced_config/parameter_reference.md#NAV_TRAFF_AVOID).
...
Before sending a vehicle command, PX4 maps the current navigator state into the same `DAA_ACTION` severity ladder and only triggers an automatic action when the requested action is to change to "a more severe mode" than the current navigator state:
...
On this page, parameters are described where their behavior matters: `NAV_TRAFF_*` in [Crosstrack Mode](#crosstrack-mode), `F34_LVL_*`, `DAA_LVL_*_ACT`, `DAA_EN_DFLT_VEL`, and `DAA_DFLT_VEL` in [F3442 Mode](#f3442-mode) and the [F3442 Volume Appendix](#f3442-volume-appendix), [DAA_NOTIF_STATE](../advanced_config/parameter_reference.md#DAA_NOTIF_STATE) in [Operator Messages](#operator-messages), [DAA_TRAFF_TOUT](../advanced_config/parameter_reference.md#DAA_TRAFF_TOUT) in [Traffic Inputs and Identification](#traffic-inputs-and-identification), and ownship ADS-B identification parameters in the same traffic-input section.
...
Under those assumptions, the expected operator-visible messages are:
...
- `spam_new` notes:
  - Note that the main conflict: `DAA Main: 34990F lvl 4. 100 m.` appears periodically during the scenario at the rate of [DAA_NOTIF_STATE](../advanced_config/parameter_reference.md#DAA_NOTIF_STATE).
