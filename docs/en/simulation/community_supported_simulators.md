...
| [Newton Air](../sim_newton_air/index.md) | <p>A GPU-accelerated drone simulation framework built on NVIDIA's Newton physics engine (with Google DeepMind and Disney Research). Uses the Warp framework (Python) for JIT-compiled simulation on GPU or CPU. PX4 runs decoupled and Newton Air connects over the simulator MAVLink API.</p><p><strong>Supported Vehicles:</strong> Quad</p>                                                                                            |
