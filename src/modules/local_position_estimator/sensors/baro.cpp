...
void BlockLocalPositionEstimator::baroInit() {
	...
	if (_baroStats.getCount() > REQ_BARO_INIT_COUNT) {
		...
		mavlink_log_info(&mavlink_log_pub, "[lpe] baro init %d m std %d cm", (int)_baroStats.getMean()(0),
...
void BlockLocalPositionEstimator::baroCorrect() {
	...
	// residual
	Matrix<float, n_y_baro, n_y_baro> S_I = inv<float, n_y_baro>((C * m_P * C.transpose()) + R);
	...
	if (beta > BETA_TABLE[n_y_baro]) {
		if (!(_sensorFault & SENSOR_BARO)) {
			mavlink_log_critical(&mavlink_log_pub, "[lpe] baro fault, r %5.2f m, beta %5.2f", double(r(0)),
					     double(beta));
			_sensorFault |= SENSOR_BARO;
		}
