...
static bool w_thread_should_exit = false;		/**< px4_uart exit flag */
static bool w_uart_thread_running = false;		/**< px4_uart status flag */
static int w_uart_task;				/**< Handle of px4_uart task / thread */
static int w_uart_init(void);
static int set_w_uart_baudrate(const int fd, unsigned int baud);

void msg_orb_sub (MSG_orb_sub *msg_fd);
void msg_orb_data(MSG_orb_data *stp_data, MSG_orb_sub stp_fd);
 ...
 */
__EXPORT int w_uart_main(int argc, char *argv[]);

/**
 * Mainloop of daemon.
 */
int w_uart_thread_main(int argc, char *argv[]);
...
int set_w_uart_baudrate(const int fd, unsigned int baud)
{
        int speed;
...
int w_uart_main(int argc, char *argv[])
{
        if (argc < 2) {
        ...
        if (!strcmp(argv[1], "start")) {
                ...
                if (w_uart_thread_running) {
                       printf("px4_uart already running\n");
                        /* this is not an error */
                        return 0;
                }

                w_thread_should_exit = false;//定义一个守护进程
                w_uart_task = px4_task_spawn_cmd("w_uart",
                        SCHED_DEFAULT,
...
{
        //char *data = "\n";
        //char sample_test_uart[]="w_uart TX-test:running!\n";
        ...
        if (false == set_w_uart_baudrate(uart_read, 115200)) {
                printf("set_w_uart_baudrate is failed\n");
                return -1;
        }
        printf("uart init is successful\n");


        w_uart_thread_running = true;

        while (!w_thread_should_exit) {
        ...
        //printf("[w_uart] exiting\n");
        w_uart_thread_running = false;
        close(uart_read);
        printf("uart close\n");
        return 0;
}
