...
def generate_px4_function(function_name, output_names):
    ...
    metadata = codegen.generate_function(
            output_dir="generated decoupled moving",
...
def computeProcessNoise(dt: sf.Scalar, _input_var: sf.Scalar, _bias_var: sf.Scalar, _acc_var: sf.Scalar) -> (MState):
...
def predictCov(dt: sf.Scalar, _input_var: sf.Scalar, _bias_var: sf.Scalar, _acc_var: sf.Scalar, _covariance: MState) -> (MState):
    Phi = sf.Matrix([  [1, dt, 0, 0.5*dt*dt],
                    [0, 1,  0, dt       ],
                    [0, 0,  1, 0        ],
                    [0, 0,  0, 1        ]])
