...
def generate_px4_function(function_name, output_names):
    ...
    metadata = codegen.generate_function(
            output_dir="src/modules/landing_target_estimator/python_derivation/generated/coupled_moving_xyz",
...
def syncState(dt: sf.Scalar, state: VState, acc: VInput) -> (VState):
    ...
    Phi = sf.Matrix.block_matrix([   [1 * identity, dt * identity, 0.5*dt*dt * identity],
                        [0 * identity, 1 * identity,  dt * identity       ],
...
def predictCov(dt: sf.Scalar, input_var: MDirections, acc_var: MDirections, covariance: MState) -> (MState):
    ...
    return G*input_var*G.T + Q_acc + Phi*covariance*Phi.T
