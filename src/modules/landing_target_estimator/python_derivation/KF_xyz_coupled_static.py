...
def generate_px4_function(function_name, output_names):
    ...
    metadata = codegen.generate_function(
            output_dir="src/modules/landing_target_estimator/python_derivation/generated/coupled_static_xyz",
...
def syncState(dt: sf.Scalar, state: VState, acc: VInput) -> (VState):
    ...
    Phi = sf.Matrix.block_matrix([   [1 * identity, dt * identity],
                        [0 * identity, 1 * identity]])

    G = sf.Matrix.block_matrix([ [-0.5*dt*dt * identity], [-dt * identity]])

    return (Phi.inv() * (state - G*acc)).simplify()

def predictCov(dt: sf.Scalar, input_var: MDirections, covariance: MState) -> (MState):
    identity = sf.Matrix([  [1, 0, 0],
                            [0, 1, 0],
                            [0, 0, 1]])
    ...
    return G*input_var*G.T + Phi*covariance*Phi.T
