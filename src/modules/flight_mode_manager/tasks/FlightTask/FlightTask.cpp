...
const trajectory_setpoint_s FlightTask::empty_setpoint = {
...
void FlightTask::_evaluateVehicleLocalPosition() {
	...
	if ((_time_stamp_current - _sub_vehicle_local_position.get().timestamp) < _timeout) {
		...
		if (_sub_vehicle_local_position.get().xy_global && _sub_vehicle_local_position.get().z_global) {
			if (!_geo_projection.isInitialized() || (_geo_projection.getProjectionReferenceTimestamp() !=
								 _sub_vehicle_local_position.get().ref_timestamp)) {
				_geo_projection.initReference(_sub_vehicle_local_position.get().ref_lat,
...
void FlightTask::_evaluateVehicleLocalPositionSetpoint() {
	...
	if (_vehicle_local_position_setpoint_sub.copy(&vehicle_local_position_setpoint) &&
		...
		// This is used to properly initialize the velocity setpoint when onpening the position loop (position
		// unlock)
		_velocity_setpoint_feedback =
			matrix::Vector3f(vehicle_local_position_setpoint.vx, vehicle_local_position_setpoint.vy,
...
void FlightTask::_evaluateDistanceToGround() {
	// Altitude above ground is local z-position or altitude above home or distance sensor altitude depending on
	// what's available
