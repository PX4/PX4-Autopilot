...
uavcan::uint64_t getUtcUSecFromCanInterrupt() { return utc_set ? sampleUtcFromCriticalSection() : 0; }
...
void adjustUtc(uavcan::UtcDuration adjustment) {
	...
	if (adjustment.getAbs() > utc_sync_params.min_jump || !utc_set) {
		...
		if (!utc_locked) {
			utc_locked = (std::abs(utc_rel_rate_ppm) < utc_sync_params.lock_thres_rate_ppm) &&
