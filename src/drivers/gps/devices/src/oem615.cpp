...
GPSDriverOEM615::GPSDriverOEM615(GPSCallbackPtr callback, void *callback_user, struct vehicle_gps_position_s *gps_position,
...
{
	decodeInit();
	_decode_state = OEM615_DECODE_UNINIT;
	_rx_buffer_bytes = 0;
}

GPSDriverOEM615::~GPSDriverOEM615(){

}

int GPSDriverOEM615::handleMessage(int len){
	char * endp;

	if (len < 7) { return 0; }

	int uiCalcComma = 0;						/* the number of msg's "," */

	for (int i = 0 ; i < len; i++) {			/* count the "," */
		if (_rx_buffer[i] == ',') { uiCalcComma++; }
	}

	char *bufptr = (char *)(_rx_buffer + 6);

//	PX4_INFO("Buf is: %s",_rx_buffer);
	int ret = 0;
	...
	if ((memcmp(_rx_buffer + 3, "ZDA,", 3) == 0) && (uiCalcComma == 6)) {
		...
		*/
		...
		int oem615_hour = static_cast<int>(oem615_time / 10000);
		int oem615_minute = static_cast<int>((oem615_time - oem615_hour * 10000) / 100);
		double oem615_sec = static_cast<float>(oem615_time - oem615_hour * 10000 - oem615_minute * 100);
		...
		timeinfo.tm_hour = oem615_hour;
		timeinfo.tm_min = oem615_minute;
		timeinfo.tm_sec = int(oem615_sec);

#ifndef NO_OEM515
		time_t epoch = mktime(&timeinfo);
		if (epoch > GPS_EPOCH_SECS) {
			uint64_t usecs = static_cast<uint64_t>((oem615_sec - static_cast<uint64_t>(oem615_sec))) * 1000000;
		...
		/*
		  ...
		  Note - The data string exceeds the OEM615 standard length.
		  GGA message fields
		  Field   Meaning
		  0   Message ID $GPGGA
...
//		if (bufptr && *(++bufptr) != ',') { units1 = *(bufptr++); }
//		if (bufptr && *(++bufptr) != ',') { alt_ellipsoid = strtod(bufptr, &endp); bufptr = endp; }
//		if (bufptr && *(++bufptr) != ',') { units2 = *(bufptr++);}
//
//		if (bufptr && *(++bufptr) != ',') { age_diff_corr = strtod(bufptr, &endp);}
...
//		_gps_position->lat = 41.0;
//		_gps_position->lon = 123.0;
//		_gps_position->alt = 1.0;

		_gps_position->satellites_used = num_of_sv;
//		PX4_INFO("num_of_sv: %d",num_of_sv);
		PX4_INFO("oem time is: %8.4f",hrt_absolute_time()*1e-6);
		...
		if (fix_quality <= 0) {
			...
			/*
			 * in this OEM615 message float integers (value 5) mode has higher value than fixed integers (value 4), whereas it provides lower quality,
...
//		_gps_position->vel_ned_valid = true;                         /**< Flag to indicate if NED speed is valid */
	...
	} else if ((memcmp(_rx_buffer, "#BESTVELA,", 9) == 0) && (uiCalcComma == 16)) {

//		bufptr = (char *)(_rx_buffer + 9);
//		double hor_spd = 0.0;
//		double trk_gnd = 0.0;
//		double vert_spd = 0.0;
//		while(bufptr && *(++bufptr) != ',')++bufptr;											/* com1 */
//		if (bufptr && *(++bufptr) != ',') { strtod(bufptr, &endp); bufptr = endp; }				/* 0 */
//		if (bufptr && *(++bufptr) != ',') { strtod(bufptr, &endp); bufptr = endp; }  			/* 61.0 */
//		while (bufptr && *(++bufptr) != ',')++bufptr;											/* FIN...ING */
//		if (bufptr && *(++bufptr) != ',') { strtod(bufptr, &endp); bufptr = endp; }				/* 1337 */
//		if (bufptr && *(++bufptr) != ',') { strtod(bufptr, &endp); bufptr = endp; }				/* 3341.00*/
//		if (bufptr && *(++bufptr) != ',') { strtod(bufptr, &endp); bufptr = endp; }  			/*000*/
//		if (bufptr && *(++bufptr) != ',') { strtod(bufptr, &endp); bufptr = endp; }  			/*827B--*/
//		if (bufptr && *(++bufptr) != ',') { strtod(bufptr, &endp); bufptr = endp; }  			/*1984*/
//		while (bufptr && *(++bufptr) != ',')++bufptr;											/* SOL_COMPUTED */
//		while (bufptr && *(++bufptr) != ',')++bufptr;											/* DOPPLER_VELOCITY */
//		if (bufptr && *(++bufptr) != ',') { strtod(bufptr, &endp); bufptr = endp; }  			/* latency */
//		if (bufptr && *(++bufptr) != ',') { strtod(bufptr, &endp); bufptr = endp; }				/* age */
//		if (bufptr && *(++bufptr) != ',') { hor_spd = strtod(bufptr, &endp); bufptr = endp; } 	/* hor spd -- m/s */
//		if (bufptr && *(++bufptr) != ',') { trk_gnd = strtod(bufptr, &endp); bufptr = endp; } 	/* trk gnd */
//		if (bufptr && *(++bufptr) != ',') { vert_spd = strtod(bufptr, &endp); bufptr = endp; } 	/* vert spd -- m/s */
//
//		float track_rad = static_cast<float>(trk_gnd) * M_PI_F / 180.0f;
//
//		float velocity_north = static_cast<float>(hor_spd) * cosf(track_rad);
//		float velocity_east  = static_cast<float>(hor_spd) * sinf(track_rad);
//
//		_gps_position->vel_m_s = hor_spd;					/** GPS ground speed (m/s) */
...
//		_gps_position->timestamp_velocity = hrt_absolute_time();
		...
		/*
		 * oem615 would return empty space as coordinate (lat, lon or alt) if it doesn't have a fix yet
		 */
		...
		*/
		double oem615_time __attribute__((unused)) = 0.0, lat_err = 0.0, lon_err = 0.0, alt_err = 0.0;
		...
		*/
		...
		if ((this_msg_num == 0) && (bGPS == true)&& _satellite_info) {
...
int GPSDriverOEM615::receive(unsigned timeout){
...
int GPSDriverOEM615::parseChar(uint8_t b){
	int iRet = 0;

	switch (_decode_state) {
		case OEM615_DECODE_UNINIT:	/* First, look for sync1 */
			if (b == '$') {
			...
			if (b == '#') {
				_decode_state = OEM615_DECODE_GOT_SYNC1;
				_rx_buffer_bytes = 0;
				_rx_buffer[_rx_buffer_bytes++] = b;
			}
			break;
		case OEM615_DECODE_GOT_SYNC1:
			if (b == '#') {				/* next msg */
				_decode_state = OEM615_DECODE_GOT_SYNC1;
				_rx_buffer_bytes = 0;
			} else if (b == '*') {		/* end of msg */
				_decode_state = OEM615_DECODE_GOT_ASTERIKS;
				_crc_pos = 0;
			}

			if (_rx_buffer_bytes >= (sizeof(_rx_buffer) - 9)) {
				_decode_state = OEM615_DECODE_UNINIT;
				_rx_buffer_bytes = 0;
			} else {					/* get msg's field */
				_rx_buffer[_rx_buffer_bytes++] = b;
			}
			break;
		case NME_DECODE_GOT_SYNC1:
			...
			break;
		case OEM615_DECODE_GOT_ASTERIKS:
			_rx_buffer[_rx_buffer_bytes++] = b;
			_crc_pos++;
			if (_crc_pos == 7)
			{
		...
		case NME_DECODE_GOT_ASTERIKS:
			_rx_buffer[_rx_buffer_bytes++] = b; /* crc1 */
			_decode_state = NME_DECODE_GOT_FIRST_CS_BYTE;
			break;
		case OEM615_DECODE_GOT_FIRST_CS_BYTE:
			_rx_buffer[_rx_buffer_bytes++] = b;
			{
				uint32_t checksum = 0;
				uint8_t *buffer = _rx_buffer + 1;					/* skip "#" */
				uint8_t *bufend = _rx_buffer + _rx_buffer_bytes - 9;/* skip *cccc */
				uint32_t buflen = bufend - buffer;
				checksum = CalculateCRC32(buflen, buffer);
				if ((HEXDIGIT_CHAR((checksum >> 4) & 0x0F) == *(_rx_buffer + _rx_buffer_bytes - 2)) &&
		...
		case NME_DECODE_GOT_FIRST_CS_BYTE:
			_rx_buffer[_rx_buffer_bytes++] = b; /* crc2 */
			{
				uint8_t checksum = 0;
				uint8_t *buffer = _rx_buffer + 1;	/* skip $ */
				uint8_t *bufend = _rx_buffer + _rx_buffer_bytes - 3;	/* skip *cc */

				for (; buffer < bufend; buffer++) { checksum ^= *buffer; }

				if ((HEXDIGIT_CHAR(checksum >> 4) == *(_rx_buffer + _rx_buffer_bytes - 2)) &&	/* *(_rx_buffer + _rx_buffer_bytes - 2)-->crc1 */
					(HEXDIGIT_CHAR(checksum & 0x0F) == *(_rx_buffer + _rx_buffer_bytes - 1))) { /* *(_rx_buffer + _rx_buffer_bytes - 1)-->crc2 */
					iRet = _rx_buffer_bytes;
				}
			}
			_decode_state = OEM615_DECODE_UNINIT;
...
uint32_t GPSDriverOEM615::CalculateCRC32(uint32_t ulcount, uint8_t *ucBuffer)
{
	uint32_t ulTemp1;
	uint32_t ulTemp2;
	uint32_t ulCRC = 0;
...
uint32_t GPSDriverOEM615::CRC32Value(int i)
{
	int j;
	uint32_t ulCRC;
	ulCRC = i;
...
int GPSDriverOEM615::configure(unsigned &baudrate, OutputMode output_mode)
{
	if (output_mode != OutputMode::GPS) {
		GPS_WARN("oem615: Unsupported Output Mode %i", (int)output_mode);
		return -1;
	}
...
void GPSDriverOEM615::decodeInit(void)
{
