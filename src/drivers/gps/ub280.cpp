	...
	if(_rx_buffer[0] == '$')
	{
		...
		if ((memcmp(_rx_buffer + 3, "ZDA,", 3) == 0) && (uiCalcComma == 6)) {
			...
			*/
			...
			int UB280_hour = static_cast<int>(UB280_time / 10000);
			int UB280_minute = static_cast<int>((UB280_time - UB280_hour * 10000) / 100);
			double UB280_sec = static_cast<float>(UB280_time - UB280_hour * 10000 - UB280_minute * 100);
			...
			timeinfo.tm_hour = UB280_hour;
			timeinfo.tm_min = UB280_minute;
			timeinfo.tm_sec = int(UB280_sec);
			time_t epoch = mktime(&timeinfo);

			if (epoch > GPS_EPOCH_SECS) {
				uint64_t usecs = static_cast<uint64_t>((UB280_sec - static_cast<uint64_t>(UB280_sec))) * 1000000;
			...
			*/
			...
			/*
			 * UB280 would return empty space as coordinate (lat, lon or alt) if it doesn't have a fix yet
			 */
		...
		*/
		...
		double UB280_time __attribute__((unused)) = 0.0, lat = 0.0, lon = 0.0, alt = 0.0;
		char *bufptr ;
		if((memcmp(_rx_buffer + 1, "BESTPOS,", 8) == 0) && (uiCalcComma == 6))
		{


			/* solve status */
			if(memcmp(_rx_buffer+now_pos,"SOL_COMPUTED",12) == 0){

			}else if(memcmp(_rx_buffer+now_pos,"INSUFFICIENT_OBS",16) == 0){

			}else if(memcmp(_rx_buffer+now_pos,"NO_CONVERGENCE",14) == 0){

			}else if(memcmp(_rx_buffer+now_pos,"COV_TRACE",9) == 0){
			...
			/* 9 latitude standard error  m  */
			while(_rx_buffer[now_pos++] != ',') if(now_pos > len ) return 0;

			/* 10 lontitude standard error m */
			while(_rx_buffer[now_pos++] != ',') if(now_pos > len ) return 0;

			/* 11 height standard error  */
			while(_rx_buffer[now_pos++] != ',') if(now_pos > len ) return 0;
			...
			/* 15 the number of tracking satellite   */
			while(_rx_buffer[now_pos++] != ',') if(now_pos > len ) return 0;

			/* 16 the number of satellite use to work out the solution  */
			while(_rx_buffer[now_pos++] != ',') if(now_pos > len ) return 0;
			...
			_gps_position->lon = static_cast<int>((int(lon * 0.01) + (lon * 0.01 - int(lon * 0.01)) * 100.0 / 60.0) * 10000000);
			_gps_position->alt = static_cast<int>(alt * 1000);
			printf("%f/n",_gps_position->lat);
		}else if((memcmp(_rx_buffer + 1, "BESTVEL,", 8) == 0) && (uiCalcComma == 6)){
			/* 2 solve status */
			if(memcmp(_rx_buffer+now_pos,"SOL_COMPUTED",12) == 0)
			{}else if(memcmp(_rx_buffer+now_pos,"INSUFFICIENT_OBS",16) == 0)
			{}else if(memcmp(_rx_buffer+now_pos,"NO_CONVERGENCE",14) == 0)
			...
			/* 7 speed direction refference truth north  deg  */
			while(_rx_buffer[now_pos++] != ',') if(now_pos > len ) return 0;

			/* 8 vertical speed +up -down  m/s  */
			while(_rx_buffer[now_pos++] != ',') if(now_pos > len ) return 0;
...
//		}
	...
	case NME_DECODE_GOT_SEVENTH_CS_BYTE:
		...
		uint32_t checksum = 0;
		uint8_t *buffer = _rx_buffer + 1;
		uint8_t *bufend = _rx_buffer + _rx_buffer_bytes - 10;

		for (; buffer < bufend; buffer++) { checksum ^= *buffer; }

		if ((HEXDIGIT_CHAR((checksum >> 24 )&0x0F) == *(_rx_buffer + _rx_buffer_bytes - 8)) &&
			(HEXDIGIT_CHAR((checksum >> 24 )&0x0F) == *(_rx_buffer + _rx_buffer_bytes - 7)) &&
			(HEXDIGIT_CHAR((checksum >> 20 )&0x0F) == *(_rx_buffer + _rx_buffer_bytes - 6)) &&
			(HEXDIGIT_CHAR((checksum >> 16 )&0x0F) == *(_rx_buffer + _rx_buffer_bytes - 5)) &&
			(HEXDIGIT_CHAR((checksum >> 12 )&0x0F) == *(_rx_buffer + _rx_buffer_bytes - 4)) &&
			(HEXDIGIT_CHAR((checksum >> 8 )&0x0F) == *(_rx_buffer + _rx_buffer_bytes - 3)) &&
			(HEXDIGIT_CHAR((checksum >> 4 )&0x0F) == *(_rx_buffer + _rx_buffer_bytes - 2)) &&
			(HEXDIGIT_CHAR(checksum & 0x0F) == *(_rx_buffer + _rx_buffer_bytes - 1))) {
			iRet = _rx_buffer_bytes;
		}
