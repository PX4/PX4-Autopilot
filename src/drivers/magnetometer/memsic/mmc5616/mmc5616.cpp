...
{
  const uint8_t chip_id = RegisterRead(Register::R_CHIP_ID);
  const uint8_t product_id = RegisterRead(Register::R_PRODUCT_ID);

  if (chip_id != EXPECTED_CHIP_ID)
  {
    PX4_DEBUG("unexpected Chip ID: 0x%02x (0x%02x)", chip_id, EXPECTED_CHIP_ID);
  }

  if (product_id != EXPECTED_PRODUCT_ID)
  {
    PX4_DEBUG("unexpected Product ID: 0x%02x (0x%02x)", product_id, EXPECTED_PRODUCT_ID);
    return false;
  }
...
MMC5616::mmc5616_measurement_t MMC5616::TakeSingleMeasurement()
{
  ...
  measurement.x = ((raw_measurement[0] - RAW_TO_SIGNED_OFFSET) * LSB_TO_MGAUSS_CONVERSION) - offset.x;
  measurement.y = ((raw_measurement[1] - RAW_TO_SIGNED_OFFSET) * LSB_TO_MGAUSS_CONVERSION) - offset.y;
  measurement.z = ((raw_measurement[2] - RAW_TO_SIGNED_OFFSET) * LSB_TO_MGAUSS_CONVERSION) - offset.z;
...
bool MMC5616::Configure()
{
  ...
  // calculate offset
  offset.x = (read_after_set.x + read_after_reset.x)/2.0f;
  offset.y = (read_after_set.y + read_after_reset.y)/2.0f;
  offset.z = (read_after_set.z + read_after_reset.z)/2.0f;
