...
{
	//static_assert(arraySize(adc_report_s::channel_id) >= NUM_CHANNELS, "TLA2528 reports 16 channels");
}
...
int TLA2528::probe()
{
	// Set device in debug mode (should respond with 0xA5A to all reads)
	...
	if(recv_data[0] == 165 && recv_data[1] == 160){
		return PX4_OK;
	}
	return PX4_ERROR;
}
