...
LSM6DSR::LSM6DSR(const I2CSPIDriverConfig &config) :
	SPI(config),
	I2CSPIDriver(config),
...
void LSM6DSR::exit_and_cleanup()
{
	DataReadyInterruptDisable();
	I2CSPIDriverBase::exit_and_cleanup();
}

void LSM6DSR::print_status()
{
	...
	PX4_INFO("Sensor ODR: %u Hz, FIFO sample dt: %.0f us",
...
void LSM6DSR::RunImpl()
{
	...
	case STATE::FIFO_READ: {
			...
			} else if (fifo_status.STATUS2 & static_cast<uint8_t>(FIFO_STATUS2_BIT::FIFO_OVR_IA)) {
				FIFOReset();
...
void LSM6DSR::ConfigureSampleRate(int sample_rate)
{
...
bool LSM6DSR::RegisterCheck(const register_config_t &reg_cfg)
{
	bool success = true;
...
uint8_t LSM6DSR::RegisterRead(Register reg)
{
	uint8_t cmd[2] {};
...
void LSM6DSR::RegisterWrite(Register reg, uint8_t value)
{
	uint8_t cmd[2] { (uint8_t)reg, value };
	transfer(cmd, cmd, sizeof(cmd));
}

void LSM6DSR::RegisterSetAndClearBits(Register reg, uint8_t setbits, uint8_t clearbits)
{
...
bool LSM6DSR::FIFORead(const hrt_abstime &timestamp_sample, uint16_t samples)
{
...
void LSM6DSR::UpdateTemperature()
{
...
int LSM6DSR::DataReadyInterruptCallback(int irq, void *context, void *arg)
{
	static_cast<LSM6DSR *>(arg)->DataReady();
	return 0;
}

void LSM6DSR::DataReady()
{
	_drdy_timestamp_sample.store(hrt_absolute_time());
	ScheduleNow();
}

bool LSM6DSR::DataReadyInterruptConfigure()
{
	if (_drdy_gpio == 0) {
		return false;
	}
...
bool LSM6DSR::DataReadyInterruptDisable()
{
	if (_drdy_gpio == 0) {
		return false;
	}
...
void LSM6DSR::ConfigureFIFOWatermark(uint8_t samples)
{
	const uint16_t fifo_watermark = samples * 2;

	for (auto &r : _register_cfg) {
		if (r.reg == Register::FIFO_CTRL1) {
			r.set_bits = fifo_watermark & 0xFF;
			r.clear_bits = static_cast<uint8_t>(~r.set_bits & 0xFF);

		} else if (r.reg == Register::FIFO_CTRL2) {
			r.set_bits = (fifo_watermark >> 8) & 0x01;
			r.clear_bits = static_cast<uint8_t>(~r.set_bits & 0x01);
		}
	}
}
