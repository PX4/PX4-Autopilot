 ...
 */
typedef enum { silUnknown = 0, silLow, silMedium, silHigh } sg_sil_t;

/**
 * Available GPS integrity SDA values
 */
typedef enum { sdaUnknown = 0, sdaMinor, sdaMajor, sdaHazardous } sg_sda_t;
...
typedef struct {
	...
	bool altUseIntrnl;       /// True = Report altitude from internal pressure sensor (will ignore other bits in the
				 /// field)
...
typedef enum { msrTypeV0, msrTypeV1Airborne, msrTypeV1Surface, msrTypeV2Airborne, msrTypeV2Surface } sg_msr_type_t;
...
typedef enum { adsbVerDO260, adsbVerDO260A, adsbVerDO260B } sg_adsb_version_t;
...
typedef enum { priNone, priGeneral, priMedical, priFuel, priComm, priUnlawful, priDowned } sg_priority_t;

typedef enum { tcrNone, tcrSingle, tcrMultiple } sg_tcr_t;
...
typedef enum { gvaUnknown, gvaLT150m, gvaLT45m } sg_gva_t;

typedef enum { nicGolham, nicNonGilham } sg_nicbaro_t;

typedef enum { svsilUnknown, svsilPow3, svsilPow5, svsilPow7 } sg_svsil_t;
...
typedef enum { trackTrueNorth, trackMagNorth, headingTrueNorth, headingMagNorth } sg_trackheading_t;

typedef enum { vrateBaroAlt, vrateGeoAlt } sg_vratetype_t;
