...
bool sgEncodeTargetReq(uint8_t *buffer, sg_targetreq_t *tgt, uint8_t msgId) {
	...
	buffer[PBASE + OFFSET_REQ_TYPE] = tgt->transmitPort << 6 | tgt->reqType;
	...
	buffer[PBASE + OFFSET_REPORTS] = tgt->ownship << 7 | tgt->commA << 6 | tgt->military << 5 | tgt->tisb << 4 |
					 tgt->airRefVel << 3 | tgt->targetState << 2 | tgt->modeStatus << 1 |
					 tgt->stateVector;
