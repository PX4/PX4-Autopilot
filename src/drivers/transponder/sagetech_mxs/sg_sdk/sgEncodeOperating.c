...
bool sgEncodeOperating(uint8_t *buffer, sg_operating_t *op, uint8_t msgId) {
	...
	buffer[PBASE + OFFSET_CONFIG] =
		op->milEmergency << 5 | op->enableXBit << 4 | op->enableSqt << 3 | op->savePowerUp << 2 | op->opMode;

	// populate Emergency/Ident
	buffer[PBASE + OFFSET_EMRG_ID] = op->identOn << 3 | op->emergcType;
