...
if [ ! -f $BASEDIR/src/platforms/nuttx/Images/$oldname.prototype ]; then
	fatal "\"$oldname\" doesn't look like an existing board name (there is no $BASEDIR/src/platforms/nuttx/Images/$oldname.prototype)"
fi
...
if [ ! -d $BASEDIR/src/platforms/nuttx/nuttx-configs/$oldname ]; then
	fatal "\"$oldname\" doesn't look like an existing board name (no such directory $BASEDIR/src/platforms/nuttx/nuttx-configs/$oldname)"
fi
...
if [ -f src/platforms/nuttx/Images/$newname.prototype -o \
  ...
  echo "rm -rf src/platforms/nuttx/Images/$newname.prototype cmake/configs/nuttx_"$newname"_default.cmake cmake/configs/nuttx_"$newname"_bootloader.cmake src/drivers/boards/$newname nuttx-configs/$newname"
  exit 1
fi
...
cp src/platforms/nuttx/Images/${oldname}.prototype src/platforms/nuttx/Images/${newname}.prototype
git add src/platforms/nuttx/Images/${newname}.prototype
...
# Make some changes to src/platforms/nuttx/Images/$(newname).prototype
sed -i -r -e 's%("(magic|description|summary)": ")([^"]*)(",).*%\1FIXME (was: \3)\4%' "src/platforms/nuttx/Images/${newname}.prototype"
