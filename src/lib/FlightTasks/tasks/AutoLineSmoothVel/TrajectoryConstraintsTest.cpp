...
TEST_F(TrajectoryConstraintsTest, testStraight)
{
	...
	float through_speed = computeStartXYSpeedFromWaypoints(vehicle_location, target, next_target, 0.f, config);

	// THEN: it should be the same as speed directly to the end point
	float direct_speed = computeStartXYSpeedFromWaypoints(vehicle_location, next_target, next_target, 0.f, config);
