...
struct Dual {
	...
	explicit Dual(Scalar v, const Vector<Scalar, N> &d) : value(v), derivative(d) {}
	...
	Dual<Scalar, N> &operator+=(const Dual<Scalar, N> &a) { return (*this = *this + a); }
...
Dual<Scalar, N> operator/(const Dual<Scalar, N> &a, const Dual<Scalar, N> &b) {
	...
	return Dual<Scalar, N>(a.value * inv_b_real,
			       a.derivative * inv_b_real - a.value * b.derivative * inv_b_real * inv_b_real);
}
...
std::ostream &operator<<(std::ostream &os, const matrix::Dual<Type, N> &dual) {
	os << "[";
