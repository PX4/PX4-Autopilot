...
TEST_F(TrajectoryConstraintsTest, test10AngleFarNext) {
	...
	// THEN: it should be the same speed as a closer next waypoint at the same angle, since the bottleneck is the
	// turn
	next_target = Vector3f(30, 11.7, 5);
...
TEST_F(TrajectoryConstraintsTest, test10AngleCloseNext) {
	...
	// THEN: it should be slower than a further next waypoint at the same angle, since the bottleneck is the
	// distance
	next_target = Vector3f(30, 11.7, 5);
