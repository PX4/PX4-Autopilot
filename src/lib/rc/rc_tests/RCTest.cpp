...
bool RCTest::crsfTest() {
	...
	while (fgets(line, line_size, fp) != nullptr) {
		if (strncmp(line, "INPUT ", 6) == 0) {
			...
			if (!has_decoded_values) {
				PX4_ERR("Test file contains decoded values but the parser did not decode anything "
					"(line=%i)",
					line_counter);
				return false;
			}
			...
			while (sscanf(file_buffer, "%x, %n", &expected_rc_value, &offset) > 0) {
				...
				if (abs(expected_rc_value - (int)rc_values[expected_num_channels]) > 10) {
					...
					ut_compare("Wrong decoded channel", expected_rc_value,
						   rc_values[expected_num_channels]);
				}
...
bool RCTest::ghstTest() {
	...
	while (fgets(line, line_size, fp) != nullptr) {
		if (strncmp(line, "INPUT ", 6) == 0) {
			...
			bool result =
				ghst_parse(now, frame, frame_len, rc_values, &ghst_rssi, &num_values, max_channels);
...
bool RCTest::dsmTest10Ch() { return dsmTest(TEST_DATA_PATH "dsm_x_data.txt", 10, 2, 1500); }

bool RCTest::dsmTest16Ch() { return dsmTest(TEST_DATA_PATH "dsm_x_dx9_data.txt", 16, 1, 1500); }

bool RCTest::dsmTest22msDSMX16Ch() { return dsmTest(TEST_DATA_PATH "dsm_x_dx9_px4_binding_data.txt", 16, 1, 1499); }

bool RCTest::dsmTestOrangeDsmx() { return dsmTest(TEST_DATA_PATH "orangerx_dsmx_12.txt", 12, 1, 1499); }
...
bool RCTest::dsmTest(const char *filepath, unsigned expected_chancount, unsigned expected_dropcount, unsigned chan0) {
	...
	while (EOF != (ret = fscanf(fp, "%f,%x,,", &f, &x))) {
		...
		// Pipe the data into the parser
		bool result = dsm_parse(f * 1e6f, &frame[0], len, rc_values, &num_values, &dsm_11_bit, &dsm_frame_drops,
		...
		if (result) {
			...
			// PX4_INFO("decoded packet with %d channels and %s encoding:", num_values, (dsm_11_bit) ? "11
			// bit" : "10 bit");
...
bool RCTest::sbus2Test() {
	...
	while (EOF != (ret = fscanf(fp, "%f,%x,,", &f, &x))) {
		...
		bool result = sbus_parse(now, &frame[0], len, rc_values, &num_values, &sbus_failsafe, &sbus_frame_drop,
...
bool RCTest::st24Test() {
	...
	while (EOF != (ret = fscanf(fp, "%f,%x,,", &f, &x))) {
		...
		if (!st24_decode(b, &rssi, &rx_count, &channel_count, channels,
