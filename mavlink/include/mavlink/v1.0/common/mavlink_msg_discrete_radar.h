...
#define MAVLINK_MSG_ID_DISCRETE_RADAR_LEN 74
#define MAVLINK_MSG_ID_107_LEN 74
...
#define MAVLINK_MESSAGE_INFO_DISCRETE_RADAR { \
	...
	{  { "time_usec", NULL, MAVLINK_TYPE_UINT64_T, 0, 0, offsetof(mavlink_discrete_radar_t, time_usec) }, \
         { "distances", NULL, MAVLINK_TYPE_INT16_T, 32, 8, offsetof(mavlink_discrete_radar_t, distances) }, \
         { "sensor_id", NULL, MAVLINK_TYPE_UINT8_T, 0, 72, offsetof(mavlink_discrete_radar_t, sensor_id) }, \
         { "quality", NULL, MAVLINK_TYPE_UINT8_T, 0, 73, offsetof(mavlink_discrete_radar_t, quality) }, \
         } \
}
...
static inline uint16_t mavlink_msg_discrete_radar_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint64_t time_usec, uint8_t sensor_id, const int16_t *distances, uint8_t quality)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[74];
	_mav_put_uint64_t(buf, 0, time_usec);
	_mav_put_uint8_t(buf, 72, sensor_id);
	_mav_put_uint8_t(buf, 73, quality);
	_mav_put_int16_t_array(buf, 8, distances, 32);
        memcpy(_MAV_PAYLOAD_NON_CONST(msg), buf, 74);
#else
	...
	mav_array_memcpy(packet.distances, distances, sizeof(int16_t)*32);
        memcpy(_MAV_PAYLOAD_NON_CONST(msg), &packet, 74);
#endif

	msg->msgid = MAVLINK_MSG_ID_DISCRETE_RADAR;
	return mavlink_finalize_message(msg, system_id, component_id, 74, 0);
}
						           ...
						           uint64_t time_usec,uint8_t sensor_id,const int16_t *distances,uint8_t quality)
{
	...
	msg->msgid = MAVLINK_MSG_ID_DISCRETE_RADAR;
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 74, 0);
}
...
{
	return mavlink_msg_discrete_radar_pack(system_id, component_id, msg, discrete_radar->time_usec, discrete_radar->sensor_id, discrete_radar->distances, discrete_radar->quality);
}
...
static inline void mavlink_msg_discrete_radar_send(mavlink_channel_t chan, uint64_t time_usec, uint8_t sensor_id, const int16_t *distances, uint8_t quality)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[74];
	_mav_put_uint64_t(buf, 0, time_usec);
	...
	_mav_put_uint8_t(buf, 73, quality);
	_mav_put_int16_t_array(buf, 8, distances, 32);
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_DISCRETE_RADAR, buf, 74, 0);
#else
	mavlink_discrete_radar_t packet;
	packet.time_usec = time_usec;
	packet.sensor_id = sensor_id;
	packet.quality = quality;
	mav_array_memcpy(packet.distances, distances, sizeof(int16_t)*32);
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_DISCRETE_RADAR, (const char *)&packet, 74, 0);
#endif
}
...
{
	return _MAV_RETURN_int16_t_array(msg, distances, 32,  8);
}
...
#else
	memcpy(discrete_radar, _MAV_PAYLOAD(msg), 74);
#endif
}
