...
#define MAVLINK_MSG_ID_GLOBAL_POSITION_TIME 127
...
#define MAVLINK_MSG_ID_127_LEN 34

#define MAVLINK_MSG_ID_GLOBAL_POSITION_TIME_CRC 237
#define MAVLINK_MSG_ID_127_CRC 237
...
#if MAVLINK_MSG_ID_GLOBAL_POSITION_TIME_LEN <= MAVLINK_MAX_PAYLOAD_LEN
/*
 ...
 */
static inline void mavlink_msg_global_position_time_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint64_t time, int32_t lat, int32_t lon, float alt, float vx, float vy, float vz, uint16_t hdg)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
#endif
#else
	mavlink_global_position_time_t *packet = (mavlink_global_position_time_t *)msgbuf;
	packet->time = time;
	packet->lat = lat;
	packet->lon = lon;
	packet->alt = alt;
