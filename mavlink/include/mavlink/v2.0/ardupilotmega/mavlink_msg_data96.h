...
#define MAVLINK_MSG_ID_DATA96_TUPLE 2, MAVLINK_MSG_ID_DATA96
#else
#define MAVLINK_MSG_ID_DATA96_TUPLE MAVLINK_MSG_ID_DATA96
#endif
...
static inline void mavlink_msg_data96_send(mavlink_channel_t chan, uint8_t type, uint8_t len, const uint8_t *data)
{
	...
	packet.len = len;
	mav_array_memcpy(packet.data, data, sizeof(uint8_t)*96);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_DATA96_TUPLE, (const char *)&packet, MAVLINK_MSG_ID_DATA96_LEN, MAVLINK_MSG_ID_DATA96_CRC);
#endif
}
...
static inline void mavlink_msg_data96_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t type, uint8_t len, const uint8_t *data)
{
	...
	_mav_put_uint8_t(buf, 1, len);
	_mav_put_uint8_t_array(buf, 2, data, 96);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_DATA96_TUPLE, buf, MAVLINK_MSG_ID_DATA96_LEN, MAVLINK_MSG_ID_DATA96_CRC);
#else
