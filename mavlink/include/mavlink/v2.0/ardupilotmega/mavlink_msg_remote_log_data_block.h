...
#define MAVLINK_MSG_ID_REMOTE_LOG_DATA_BLOCK_TUPLE 2, MAVLINK_MSG_ID_REMOTE_LOG_DATA_BLOCK
#else
#define MAVLINK_MSG_ID_REMOTE_LOG_DATA_BLOCK_TUPLE MAVLINK_MSG_ID_REMOTE_LOG_DATA_BLOCK
#endif
...
static inline void mavlink_msg_remote_log_data_block_send(mavlink_channel_t chan, uint8_t target_system, uint8_t target_component, uint32_t seqno, const uint8_t *data)
{
	...
	packet.target_component = target_component;
	mav_array_memcpy(packet.data, data, sizeof(uint8_t)*200);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_REMOTE_LOG_DATA_BLOCK_TUPLE, (const char *)&packet, MAVLINK_MSG_ID_REMOTE_LOG_DATA_BLOCK_LEN, MAVLINK_MSG_ID_REMOTE_LOG_DATA_BLOCK_CRC);
#endif
}
...
static inline void mavlink_msg_remote_log_data_block_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t target_system, uint8_t target_component, uint32_t seqno, const uint8_t *data)
{
	...
	_mav_put_uint8_t(buf, 5, target_component);
	_mav_put_uint8_t_array(buf, 6, data, 200);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_REMOTE_LOG_DATA_BLOCK_TUPLE, buf, MAVLINK_MSG_ID_REMOTE_LOG_DATA_BLOCK_LEN, MAVLINK_MSG_ID_REMOTE_LOG_DATA_BLOCK_CRC);
#else
