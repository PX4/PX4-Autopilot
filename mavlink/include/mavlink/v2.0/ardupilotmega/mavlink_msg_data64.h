...
#define MAVLINK_MSG_ID_DATA64_TUPLE 2, MAVLINK_MSG_ID_DATA64
#else
#define MAVLINK_MSG_ID_DATA64_TUPLE MAVLINK_MSG_ID_DATA64
#endif
...
static inline void mavlink_msg_data64_send(mavlink_channel_t chan, uint8_t type, uint8_t len, const uint8_t *data)
{
	...
	packet.len = len;
	mav_array_memcpy(packet.data, data, sizeof(uint8_t)*64);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_DATA64_TUPLE, (const char *)&packet, MAVLINK_MSG_ID_DATA64_LEN, MAVLINK_MSG_ID_DATA64_CRC);
#endif
}
...
static inline void mavlink_msg_data64_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t type, uint8_t len, const uint8_t *data)
{
	...
	_mav_put_uint8_t(buf, 1, len);
	_mav_put_uint8_t_array(buf, 2, data, 64);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_DATA64_TUPLE, buf, MAVLINK_MSG_ID_DATA64_LEN, MAVLINK_MSG_ID_DATA64_CRC);
#else
