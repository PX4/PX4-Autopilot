...
#define MAVLINK_MSG_ID_GPS_INJECT_DATA_TUPLE 0, MAVLINK_MSG_ID_GPS_INJECT_DATA
#else
#define MAVLINK_MSG_ID_GPS_INJECT_DATA_TUPLE MAVLINK_MSG_ID_GPS_INJECT_DATA
#endif
...
static inline void mavlink_msg_gps_inject_data_send(mavlink_channel_t chan, uint8_t target_system, uint8_t target_component, uint8_t len, const uint8_t *data)
{
	...
	packet.len = len;
	mav_array_memcpy(packet.data, data, sizeof(uint8_t)*110);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_GPS_INJECT_DATA_TUPLE, (const char *)&packet, MAVLINK_MSG_ID_GPS_INJECT_DATA_LEN, MAVLINK_MSG_ID_GPS_INJECT_DATA_CRC);
#endif
}
...
static inline void mavlink_msg_gps_inject_data_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t target_system, uint8_t target_component, uint8_t len, const uint8_t *data)
{
	...
	_mav_put_uint8_t(buf, 2, len);
	_mav_put_uint8_t_array(buf, 3, data, 110);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_GPS_INJECT_DATA_TUPLE, buf, MAVLINK_MSG_ID_GPS_INJECT_DATA_LEN, MAVLINK_MSG_ID_GPS_INJECT_DATA_CRC);
#else
