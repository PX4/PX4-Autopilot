...
#define MAVLINK_MSG_ID_DEBUG_VECT_TUPLE 0, MAVLINK_MSG_ID_DEBUG_VECT
#else
#define MAVLINK_MSG_ID_DEBUG_VECT_TUPLE MAVLINK_MSG_ID_DEBUG_VECT
#endif
...
static inline void mavlink_msg_debug_vect_send(mavlink_channel_t chan, const char *name, uint64_t time_usec, float x, float y, float z)
{
	...
	packet.z = z;
	mav_array_memcpy(packet.name, name, sizeof(char)*10);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_DEBUG_VECT_TUPLE, (const char *)&packet, MAVLINK_MSG_ID_DEBUG_VECT_LEN, MAVLINK_MSG_ID_DEBUG_VECT_CRC);
#endif
}
...
static inline void mavlink_msg_debug_vect_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  const char *name, uint64_t time_usec, float x, float y, float z)
{
	...
	_mav_put_float(buf, 16, z);
	_mav_put_char_array(buf, 20, name, 10);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_DEBUG_VECT_TUPLE, buf, MAVLINK_MSG_ID_DEBUG_VECT_LEN, MAVLINK_MSG_ID_DEBUG_VECT_CRC);
#else
