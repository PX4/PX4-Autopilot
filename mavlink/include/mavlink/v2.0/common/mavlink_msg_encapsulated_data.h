...
#define MAVLINK_MSG_ID_ENCAPSULATED_DATA_TUPLE 0, MAVLINK_MSG_ID_ENCAPSULATED_DATA
#else
#define MAVLINK_MSG_ID_ENCAPSULATED_DATA_TUPLE MAVLINK_MSG_ID_ENCAPSULATED_DATA
#endif
...
static inline void mavlink_msg_encapsulated_data_send(mavlink_channel_t chan, uint16_t seqnr, const uint8_t *data)
{
	...
	packet.seqnr = seqnr;
	mav_array_memcpy(packet.data, data, sizeof(uint8_t)*253);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_ENCAPSULATED_DATA_TUPLE, (const char *)&packet, MAVLINK_MSG_ID_ENCAPSULATED_DATA_LEN, MAVLINK_MSG_ID_ENCAPSULATED_DATA_CRC);
#endif
}
...
static inline void mavlink_msg_encapsulated_data_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint16_t seqnr, const uint8_t *data)
{
	...
	_mav_put_uint16_t(buf, 0, seqnr);
	_mav_put_uint8_t_array(buf, 2, data, 253);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_ENCAPSULATED_DATA_TUPLE, buf, MAVLINK_MSG_ID_ENCAPSULATED_DATA_LEN, MAVLINK_MSG_ID_ENCAPSULATED_DATA_CRC);
#else
