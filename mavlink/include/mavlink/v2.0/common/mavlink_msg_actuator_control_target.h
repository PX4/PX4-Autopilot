...
#define MAVLINK_MSG_ID_ACTUATOR_CONTROL_TARGET_TUPLE 0, MAVLINK_MSG_ID_ACTUATOR_CONTROL_TARGET
#else
#define MAVLINK_MSG_ID_ACTUATOR_CONTROL_TARGET_TUPLE MAVLINK_MSG_ID_ACTUATOR_CONTROL_TARGET
#endif
...
static inline void mavlink_msg_actuator_control_target_send(mavlink_channel_t chan, uint64_t time_usec, uint8_t group_mlx, const float *controls)
{
	...
	packet.group_mlx = group_mlx;
	mav_array_memcpy(packet.controls, controls, sizeof(float)*8);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_ACTUATOR_CONTROL_TARGET_TUPLE, (const char *)&packet, MAVLINK_MSG_ID_ACTUATOR_CONTROL_TARGET_LEN, MAVLINK_MSG_ID_ACTUATOR_CONTROL_TARGET_CRC);
#endif
}
...
static inline void mavlink_msg_actuator_control_target_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint64_t time_usec, uint8_t group_mlx, const float *controls)
{
	...
	_mav_put_uint8_t(buf, 40, group_mlx);
	_mav_put_float_array(buf, 8, controls, 8);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_ACTUATOR_CONTROL_TARGET_TUPLE, buf, MAVLINK_MSG_ID_ACTUATOR_CONTROL_TARGET_LEN, MAVLINK_MSG_ID_ACTUATOR_CONTROL_TARGET_CRC);
#else
