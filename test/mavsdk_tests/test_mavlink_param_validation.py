...
Verifies that PX4 rejects mission items and commands carrying non-zero values
in parameter slots that the MAVLink spec marks as unsupported for that MAV_CMD,
and accepts identical items/commands with those slots zeroed or NaN.
...
def _check(label: str, got, expected) -> bool:
    ...
    expected_name = _mission_result_name(expected) if isinstance(expected, int) else expected
    got_name = _mission_result_name(got) if isinstance(got, int) else got
    print(f"  [{status}] {label}")
    if not ok:
        print(f"          got={got_name} ({got}), expected={expected_name} ({expected})")
    return ok
...
def connect(url: str) -> "mavutil.mavfile":
...
def _upload_mission(mav, items: list[dict], timeout: float) -> int | None:
    """
    Run the MISSION_ITEM_INT upload protocol and return the MAV_MISSION_RESULT
    from the final MISSION_ACK, or None on timeout.
    """
    mav.mav.mission_count_send(
        mav.target_system, mav.target_component, len(items), 0  # mission_type=0 (main)
    )
...
def _send_command(mav, cmd: int, timeout: float,
                  p1=0.0, p2=0.0, p3=0.0, p4=0.0,
                  p5=0.0, p6=0.0, p7=0.0) -> int | None:
    ...
    # Drain until we get an ACK for this specific command
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        msg = mav.recv_match(type="COMMAND_ACK", blocking=True, timeout=timeout)
...
def run_mission_tests(mav, timeout: float) -> None:
    ...
    # mask 0x0B → p1 (hold), p2 (accept_radius), p4 (yaw) supported; p3 NOT supported.
    ...
    result = _upload_mission(mav, [
        _item(0, CMD_NAV_WAYPOINT, MAV_FRAME_GLOBAL_INT,
              p1=0.0, p2=2.0, p3=1.0, p4=NAN,   # p3 not in mask 0x0B
              x=LAT, y=LON, z=ALT, current=1),
    ], timeout)
    _check("NAV_WAYPOINT unsupported param3 set → INVALID_PARAM3",
    ...
    _check("NAV_RTL unsupported param1 set → INVALID_PARAM1",
           result, MAV_MISSION_INVALID_PARAM1)

    # --- 4. NAV_RTL with all params NaN (mask 0x00 — should pass) ---
    result = _upload_mission(mav, [
        _item(0, CMD_NAV_RTL, MAV_FRAME_GLOBAL_INT,
              p1=NAN, p2=NAN, p3=NAN, p4=NAN,
              x=LAT, y=LON, z=ALT, current=1),
    ], timeout)
    _check("NAV_RTL all params NaN → ACCEPTED", result, MAV_MISSION_ACCEPTED)
    ...
    result = _upload_mission(mav, [
        _item(0, CMD_NAV_DELAY, MAV_FRAME_MISSION,
              ...
              x=42, y=0, z=0.0),   # x != 0 → invalid param5
    ], timeout)
    _check("NAV_DELAY unsupported param5 (x=42) → INVALID_PARAM5",
    ...
    _check("NAV_DELAY valid (p1=10, p5/p6/p7=0) → ACCEPTED",
           result, MAV_MISSION_ACCEPTED)

    # --- 7. NAV_TAKEOFF with unsupported param2 set (mask 0x08 → only p4 supported) ---
    result = _upload_mission(mav, [
        _item(0, CMD_NAV_TAKEOFF, MAV_FRAME_GLOBAL_INT,
              p1=NAN, p2=5.0, p3=NAN, p4=NAN,   # p2 not in mask 0x08
              x=LAT, y=LON, z=ALT, current=1),
    ], timeout)
    _check("NAV_TAKEOFF unsupported param2 set → INVALID_PARAM2",
           result, MAV_MISSION_INVALID_PARAM2)


def run_command_tests(mav, timeout: float) -> None:
    print("\n=== Command (COMMAND_LONG) tests ===")

    # --- 8. Valid COMPONENT_ARM_DISARM (p1=0 disarm, p2=0 no-force) ---
    result = _send_command(mav, CMD_COMPONENT_ARM_DISARM, timeout, p1=0.0, p2=0.0)
    _check("COMPONENT_ARM_DISARM valid params → not DENIED",
           result != MAV_RESULT_DENIED, True)

    # --- 9. COMPONENT_ARM_DISARM with unsupported param3 set (mask 0x03) ---
    result = _send_command(mav, CMD_COMPONENT_ARM_DISARM, timeout,
                           p1=0.0, p2=0.0, p3=1.0)
    _check("COMPONENT_ARM_DISARM unsupported param3 set → DENIED",
           result, MAV_RESULT_DENIED)

    # --- 10. NAV_RTL command with param1 set (mask 0x00) ---
    result = _send_command(mav, CMD_NAV_RTL, timeout, p1=1.0)
    _check("NAV_RTL command unsupported param1 set → DENIED",
           result, MAV_RESULT_DENIED)

    # --- 11. NAV_RTL command all params zero → not DENIED ---
    result = _send_command(mav, CMD_NAV_RTL, timeout)
    _check("NAV_RTL command all params zero → not DENIED",
           result != MAV_RESULT_DENIED, True)

    # --- 12. NAV_WAYPOINT command unsupported param3 (mask 0x0B) ---
    result = _send_command(mav, CMD_NAV_WAYPOINT, timeout,
                           p1=0.0, p2=2.0, p3=1.0, p4=0.0)
    _check("NAV_WAYPOINT command unsupported param3 set → DENIED",
           result, MAV_RESULT_DENIED)
...
def main() -> int:
    ...
    parser.add_argument("--url", default="udp://:14540",
