    ...
    stage('Analysis') {
      ...
      parallel {
        ...
        stage('Catkin build on ROS workspace') {
          agent {
            docker {
              image 'px4io/px4-dev-ros-melodic:2020-01-10'
        ...
        stage('Colcon build on ROS2 workspace') {
          agent {
            docker {
              image 'px4io/px4-dev-ros2-dashing:2020-01-10'
              args '-e CCACHE_BASEDIR=$WORKSPACE -v ${CCACHE_DIR}:${CCACHE_DIR}:rw -e HOME=$WORKSPACE'
            }
          }
          steps {
        ...
        stage('Style check') {
          agent {
            docker { image 'px4io/px4-dev-base-bionic:2020-01-10' }
          }
          steps {
        ...
        stage('px4_io-v2 (bloaty)') {
          agent {
            docker {
              image 'px4io/px4-dev-nuttx:2020-01-10'
        ...
        stage('SITL unit tests') {
          agent {
            docker {
              image 'px4io/px4-dev-base-bionic:2020-01-10'
              args '-e CCACHE_BASEDIR=$WORKSPACE -v ${CCACHE_DIR}:${CCACHE_DIR}:rw'
            }
          }
          steps {
        ...
        stage('Clang analyzer') {
          agent {
            docker {
              image 'px4io/px4-dev-clang:2020-01-10'
              args '-e CCACHE_BASEDIR=$WORKSPACE -v ${CCACHE_DIR}:${CCACHE_DIR}:rw'
            }
          }
          steps {
