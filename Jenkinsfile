  ...
  stages {
    ...
    stage('Analysis') {
      ...
      parallel {
        ...
        //   agent {
        //     docker {
        //       image 'px4io/px4-dev-ros-melodic:2021-04-26'
        ...
        stage('Colcon build on ROS2 workspace') {
          agent {
            docker {
              image 'px4io/px4-dev-ros2-foxy:2021-04-26'
              args '-e CCACHE_BASEDIR=$WORKSPACE -v ${CCACHE_DIR}:${CCACHE_DIR}:rw -e HOME=$WORKSPACE'
            }
          }
          steps {
        ...
        stage('Airframe') {
          agent {
            docker { image 'px4io/px4-dev-base-focal:2021-04-26' }
          }
          steps {
            sh 'make distclean'
        ...
        stage('uORB graphs') {
          agent {
            docker {
              image 'px4io/px4-dev-nuttx-focal:2021-04-26'
