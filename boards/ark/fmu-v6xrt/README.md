...
- **LSM6DSV80X — high-g accel channel, and the reason the board has a heater.** The driver publishes the part's ±80 g high-g accelerometer, not its ±16 g low-g one, so the range never changes mid-flight. That channel's zero-g offset is ±1.5 g typical (vs ±12 mg on the low-g channel), so an uncalibrated board reports `Accel 1 inconsistent`; a six-orientation accel calibration fixes that. That calibration is only good near the die temperature it was done at: the offset moves **~2 mg/°C, about 30× the other two IMUs**, so a 10 °C excursion from the calibration temperature is 0.2 m/s² of bias, over a quarter of the `COM_ARM_IMU_ACC` limit. `HEATER1_SENS_ID` targets this die for that reason. Calibrate only once `heater status` reports the target met, fly at the same setpoint, and expect `Accel 1 inconsistent` on a cold board until the heater catches up. The 80X and 320X share `WHO_AM_I`, which is why the installed part is declared with `-T`.
