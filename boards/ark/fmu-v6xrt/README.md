...
- **LSM6DSV80X — high-g accel channel.** The driver publishes the part's ±80 g high-g accelerometer, not its ±16 g low-g one, so the range never changes mid-flight. That channel's zero-g offset is ±1.5 g typical (vs ±12 mg on the low-g channel) and drifts ~2 mg/°C, so an uncalibrated board reports `Accel 1 inconsistent` and the heater (`HEATER1_SENS_ID`) targets this die. Run a six-orientation accel calibration before flight. The 80X and 320X share `WHO_AM_I`, which is why the installed part is declared with `-T`.
