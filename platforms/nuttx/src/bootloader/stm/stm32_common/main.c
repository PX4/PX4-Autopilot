...
typedef enum mcu_rev_e { MCU_REV_Z = 0x1001, MCU_REV_Y = 0x1003, MCU_REV_X = 0x2001, MCU_REV_V = 0x2003 } mcu_rev_e;
...
static bool board_test_usart_receiving_break(void) {
	 ...
	 * Set period to 4.34 microseconds (timer_period = timer_tick / timer_reset_frequency = 168MHz / (1/4.34us) =
	 * 729.12 ~= 729)
	 */
	...
	/* Loop for 3 transmission byte cycles and count the low and high bits. Sampled at a rate to be able to count
	 * each bit twice.
	 *
...
inline void arch_setvtor(const uint32_t *address) { putreg32((uint32_t)address, NVIC_VECTAB); }
...
uint32_t flash_func_read_otp(uintptr_t address) { return 0; }

uint32_t get_mcu_id(void) { return *(uint32_t *)STM32_DEBUGMCU_BASE; }
...
int check_silicon(void) { return 0; }
