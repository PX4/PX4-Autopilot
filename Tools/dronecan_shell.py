...
def scan_nodes(node):
    ...
    for node_id in nodes:
        node.request(dronecan.uavcan.protocol.GetNodeInfo.Request(), node_id,
                     lambda e, nid=node_id: e and nodes.__setitem__(nid, bytes(e.response.name).decode('utf-8', errors='replace')),
                     timeout=TIMEOUT)
...
def terminal(node, target_node_id):
    ...
    def send(data, flags=0):
        ...
        def on_response(event):
            ...
            if event.response.flags & RSP_SHELL_ERROR:
                sys.stdout.write("\r\n[shell error, Ctrl-C starts a new one]\r\n")
                sys.stdout.flush()
