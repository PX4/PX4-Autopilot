...
# Parameters supported by the installed st_folium, resolved once so the
# version-compat wrapper does not introspect the signature on every rerun.
_ST_FOLIUM_PARAMS = set(inspect.signature(st_folium).parameters)
...
def st_folium_compat(
    ...
    *,
    ...
    feature_group_to_add: Optional[folium.FeatureGroup] = None,
    returned_objects: Optional[Sequence[str]] = None,
) -> Dict:
    """Call st_folium, passing only the arguments the installed version supports.

    ``center``/``zoom`` drive the view imperatively: st_folium only pans/zooms
    when these values change, rather than reloading the map. ``feature_group_to_add``
    layers dynamic markers on top of the base map without remounting it. On
    versions too old to support feature groups, the markers are baked into the
    base map instead so they still render.
    """
    kwargs: Dict = {}
    if "key" in _ST_FOLIUM_PARAMS:
        kwargs["key"] = key
    if "height" in _ST_FOLIUM_PARAMS:
        kwargs["height"] = height
    if "use_container_width" in _ST_FOLIUM_PARAMS:
        kwargs["use_container_width"] = True
    if returned_objects is not None and "returned_objects" in _ST_FOLIUM_PARAMS:
        kwargs["returned_objects"] = list(returned_objects)
    if center is not None and "center" in _ST_FOLIUM_PARAMS:
        kwargs["center"] = center
    if zoom is not None and "zoom" in _ST_FOLIUM_PARAMS:
        kwargs["zoom"] = zoom
    if feature_group_to_add is not None:
        if "feature_group_to_add" in _ST_FOLIUM_PARAMS:
            kwargs["feature_group_to_add"] = feature_group_to_add
        else:
            feature_group_to_add.add_to(m)
...
def _load_default_numeric_vars() -> Dict[str, float]:
    ...
    helper_path = (
        ...
        / "src/modules/navigator/test/support/mission_route_test_helpers.h"
    )
    numeric_vars: Dict[str, float] = {}
    try:
...
def _extract_variable_definitions(
    ...
    """Extract numeric constants and current or legacy position variables."""
    ...
    # Constants in the pasted fixture override the shared helper defaults.
    for name, _ in numeric_defs:
        numeric_vars.pop(name, None)
    ...
    latlon_var_pattern = re.compile(
        ...
        r"(?:mission_route::Position|Mission::LatLonAlt|MissionRoutePlanner::Position)"
        r"\s+(\w+)\s*(?:=\s*)?\{([^{}]+)\}\s*;",
        re.MULTILINE,
    )
...
def _parse_legacy_mission_arrays(src: str) -> Dict[str, List[Waypoint]]:
    ...
    mission_array_pattern = re.compile(
        r"(?:static\s+)?(?:constexpr\s+)?(?:const\s+)?std::array<\s*MissionItemInput\s*,\s*\d+\s*>\s+(\w+)\s*\{\s*\{(.*?)\}\s*\}\s*;",
...
def _parse_latlon_points(
    ...
    """Parse position helpers and current or legacy unnamed position literals."""
    ...
    position_literal_pattern = re.compile(
        r"(?:mission_route::Position|MissionRoutePlanner::Position)\s*\{([^{}]+)\}"
    )
...
def _add_velocity_arrow_to_map(
    m: Union[folium.Map, folium.FeatureGroup],
    lat: float,
    lon: float,
    vel_n: float,
    vel_e: float,
...
def _build_base_map(
    ...
    """Build the static base map: tiles, the lat/lon click popup, and the parsed
    reference layers.

    The interactive creator points are deliberately *not* drawn here; they are
    layered on separately via ``feature_group_to_add``. st_folium remounts the
    Leaflet map (resetting the user's pan/zoom) whenever this base-map HTML
    changes, so keeping it stable between clicks is what stops the view from
    snapping back.
    """
...
def _build_creator_feature_group(
    creator_points: CreatorPointsMap,
    active_mode: str,
) -> folium.FeatureGroup:
    """Build the FeatureGroup of interactive "creator" points.

    Returned separately from the base map so st_folium can update it in place
    (via ``feature_group_to_add``) without remounting the map and resetting the
    user's pan/zoom.
    """
    fg = folium.FeatureGroup(name="creator")
    ...
    for mode, points in creator_points.items():
        ...
        if mode == "Mission" and len(points) >= 2:
            folium.PolyLine(coords_2d, color=color, weight=weight, opacity=opacity).add_to(fg)

        elif "Polygon" in mode:
            if len(points) >= 3:
                folium.Polygon(coords_2d, color=color, fill=True, fill_opacity=0.2).add_to(fg)
            elif len(points) >= 2:
                folium.PolyLine(coords_2d, color=color, weight=weight).add_to(fg)
            ...
            for i in range(0, len(coords_2d)-1, 2):
                ...
                folium.PolyLine(seg, color=color, weight=weight, dash_array="5, 10").add_to(fg)
...
def main():
    ...
    with main_tab:
        ...
        with col_map:
            # Map Rendering.
            #
            # st_folium remounts the Leaflet map (and resets the view to the
            # map's defaults) whenever the *base-map* HTML changes. To keep
            # zooming/panning stable:
            #   - the base map's center/zoom are held fixed and only change on an
            #     explicit re-center (e.g. parsing new code), so editing points
            #     never rebuilds the base map;
            #   - the interactive creator points ride on a feature group, which
            #     st_folium updates in place without remounting;
            #   - center/zoom are passed as arguments (st_folium only moves the
            #     view when they change) and are *not* requested back in
            #     returned_objects, so a plain pan/zoom no longer triggers a
            #     rerun that fights the user.
            center = tuple(st.session_state["creator_map_center"])
            zoom = st.session_state["creator_map_zoom"]
            parsed_data = st.session_state["parse_result"]

            base_map = _build_base_map(parsed_data, center, zoom)
            creator_fg = _build_creator_feature_group(
                st.session_state["creator_points"], mode
            )
            ...
            ret = st_folium_compat(
                ...
                center=center,
                zoom=zoom,
                feature_group_to_add=creator_fg,
