...
parser.add_argument("-s", "--send", dest='send', metavar='*', type=str, nargs='+', help="Topics to be sent")
parser.add_argument("-r", "--receive", dest='receive', metavar='*', type=str, nargs='+', help="Topics to be received")
parser.add_argument("-t", "--topic-msg-dir", dest='msgdir', type=str, nargs=1, help="Topics message dir")
parser.add_argument("-o", "--outdir", dest='outdir', type=str, nargs=1, help="Topics message dir")

print(sys.argv)
if len(sys.argv) > 1:
    msg_folder = parser.parse_args().msgdir[0]
    msg_files_send = [ msg_folder+"/"+x+".msg" for x in parser.parse_args().send ]
    msg_files_receive = [ msg_folder+"/"+x+".msg" for x in parser.parse_args().receive ]
else:
    parser.print_usage()
    exit(-1)
...
uRTPS_CLIENT_TEMPL_FILE = 'microRTPS_UDP_client.cpp.template'
